...
当前 code block 使用浅薰衣草底色 + 深紫色文字的高对比度方案。
行内代码使用紫色文字 + 半透明背景。
...
- **字体**：默认使用系统字体（Segoe UI / PingFang SC），可在 `style.css` 中替换
- **头像**：首页头像目前是 🌸 emoji，可替换为本地图片（修改 `index.html` 中 `.avatar-inner` 的内容）
- **浮动光点**：首页的动画粒子在 `index.html` 的 `.sparkle-container` 中，可增删或调整位置
